...
export async function renderMediaLibraryGrid() {
  ...
  try {
    ...
    if (mediaList.length === 0) {
      grid.innerHTML = '<div style="font-size: 0.75rem; color: var(--text-muted); grid-column: 1/-1; text-align: center; padding: 1rem;">Nenhuma mídia enviada ainda. Faça o upload acima!</div>';
      return;
    }
    ...
    mediaList.forEach(item => {
      ...
      const mediaPreview = isVideo
        ? `<div style="height: 80px; background: #000; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 0.7rem;">🎬 Vídeo</div>`
        : `<img src="${item.url}" alt="${item.original_name}" style="height: 80px; width: 100%; object-fit: cover;">`;
      ...
      card.innerHTML = `
        ${mediaPreview}
        <div style="padding: 0.4rem; font-size: 0.65rem; flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
          ...
          <div style="display: flex; gap: 0.25rem;">
            ...
            <button type="button" class="btn-delete-media" data-id="${item.id}" style="background: none; border: 1px solid var(--border-color); color: #F43F5E; border-radius: 4px; padding: 0.2rem 0.3rem; font-size: 0.6rem; cursor: pointer;">🗑️</button>
          </div>
        </div>
      `;
